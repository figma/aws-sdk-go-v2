...
func TestResolveBaseEndpoint(t *testing.T) {
	cases := map[string]struct {
		...
		"env ignore": {
			...
			envService:    "https://env-json10querycompatible.dev",
			envIgnore:     true,
			configGlobal:  "http://config-global.dev",
			configService: "http://config-json10querycompatible.dev",
			expectURL:     nil,
		},
		"env global": {
			envGlobal:     "https://env-global.dev",
		...
		"env service": {
			...
			configGlobal:  "http://config-global.dev",
			configService: "http://config-json10querycompatible.dev",
			expectURL:     aws.String("https://env-json10querycompatible.dev"),
		},
		"config ignore": {
			envGlobal:     "https://env-global.dev",
			envService:    "https://env-json10querycompatible.dev",
		...
		"config service": {
			configGlobal:  "http://config-global.dev",
			configService: "http://config-json10querycompatible.dev",
			expectURL:     aws.String("http://config-json10querycompatible.dev"),
		},
		"client": {
			envGlobal:      "https://env-global.dev",
			envService:     "https://env-json10querycompatible.dev",
			configGlobal:   "http://config-global.dev",
			configService:  "http://config-json10querycompatible.dev",
			clientEndpoint: aws.String("https://client-json10querycompatible.dev"),
			expectURL:      aws.String("https://client-json10querycompatible.dev"),
		},
	}
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if c.envService != "" {
				t.Setenv("AWS_ENDPOINT_URL_JSON10QUERYCOMPATIBLE", c.envService)
			}
