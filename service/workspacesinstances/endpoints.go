...
import (
	...
	internalendpoints "github.com/figma/aws-sdk-go-v2/service/workspacesinstances/internal/endpoints"
...
func resolveBaseEndpoint(cfg aws.Config, o *Options) {
	...
	_, s := os.LookupEnv("AWS_ENDPOINT_URL_WORKSPACES_INSTANCES")

	if g && !s {
		return
	}

	value, found, err := internalConfig.ResolveServiceBaseEndpoint(context.Background(), "Workspaces Instances", cfg.ConfigSources)
	if found && err == nil {
		...
		if exprVal := awsrulesfn.GetPartition(_Region); exprVal != nil {
			...
			if _UseFIPS == true {
				uriString := func() string {
					...
					out.WriteString("https://workspaces-instances-fips.")
			...
			uriString := func() string {
				var out strings.Builder
				out.WriteString("https://workspaces-instances.")
				out.WriteString(_Region)
				out.WriteString(".")
				out.WriteString(_PartitionResult.DualStackDnsSuffix)
				return out.String()
